...
<br/>
<form method="post" action="{{settings.PREFIX }}/search/computer/" name="usersearch">
<table>
<tr>
...
<br/>
%if defined('query'):
<h2>Результаты поиска компьютеры:</h2>
<table border=1>
<tr><th>Компьютер</th><th>Включен</th><th>Выключен</th><th>Длительность</th></td>
%for i in query:
<tr><td>{{i[1].hostname}}</td><td>{{i[0].session_start}}</td><td>{{i[0].session_end}}</td><td>{{i[0].session_end - i[0].session_start}}</td></tr>
%end
</table>
%end
