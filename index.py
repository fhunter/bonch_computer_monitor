...
def main():
    ...
    try:
        data = requests.get(
            "https://gitea.pivt.spbgut.ru/api/v1/repos/PIVT/bonch_ansible/branches/master",
    ...
    for i in rooms:
        ...
        for record in result:
            userslog[record.machineid] = session_user.get_active_users(
...
def search():
    ...
    session = Session()
    start_date = (
        session.query(UserSession.session_start)
        .order_by(UserSession.session_start.asc())
        .limit(1)
        .one()[0]
        .strftime("%Y-%m-%d")
    )
...
def search():
    ...
    start_date = (
        ...
        .order_by(ComputerSession.session_start.asc())
        .limit(1)
        .one()[0]
        .strftime("%Y-%m-%d")
    )
...
def search():
    ...
    end_none = now + datetime.timedelta(365)  # year forward
    result = session.query(ComputerSession, Computer).join(
    ...
    result = result.filter(
        func.coalesce(ComputerSession.session_end, end_none) >= startdate
    )
    result = result.all()
    session.close()
    return dict(
...
@app.route(
    settings.PREFIX + "/graph/<grp:re:[i,g]>/<hostname>_<typ>/<period:re:[d,w,m,y]>"
)
...
def acceptansibledata():
    ...
    try:
        ...
        rrd_ansible.insert(
            hostname, [ok_value, change_value, unreachable_value, failed_value]
        )
...
if __name__ == "__main__":
    bottle.run(app, host="127.0.0.1", port=8086, reloader=True)
